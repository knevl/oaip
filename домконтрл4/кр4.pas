...
procedure interval;{Процедура интервала}
  begin
    repeat
      a := readlnreal('Введите левую границу интервала от -2 до 0 a = ');
    until (a >= -2) and (a <= 0);
    repeat
...
procedure grafik(var gx, gy, x0, y0, k, x1, y1, i, e, r, x2: integer; x, y, h: real); {процедура графика}
begin
  gx := 800;
  gy := 1500;
  k := 20;
  ...
  while x < b do
  begin
    y := 2 * power(x, 3) + 2 * power(x, 2) -2 * x + 19;
    x1 := x0 + round(x * k);
    y1 := y0 - round(y * k);
    lineTo(x1, y1);
    x += 1 / n;
